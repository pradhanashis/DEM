function [len]= Degree_270(i,j, ip_ang, feature)
    ...
    n=0;
        ...
        if i <= (r-1) %IF CURRENT LOCATION IS NOT BORDER
        ...
        while (flag~=1)
                if i>=(r-1)   %Stopping Criteria for if traversal reaches border of image 
                    flag=1;
                    border=1;                
                    len=0;
                    break;
                ...
                if tmp(5)==0
                        len=len+1;
                        i=i+1;  j=j; %Update coodinate value
                end
